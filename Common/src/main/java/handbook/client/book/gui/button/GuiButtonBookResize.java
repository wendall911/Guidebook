...
public class GuiButtonBookResize extends GuiButtonBook {
	...
	public GuiButtonBookResize(GuiBook parent, int x, int y, Button.OnPress onPress) {
		super(parent, x, y, 330, 9, 11, 11, onPress,
				Component.translatable("handbook.gui.lexicon.button.resize"));
	}
	...
	public List<Component> getTooltipLines() {
		...
		if (size <= lastSize) {
			sizeMsg = Component.translatable("handbook.gui.lexicon.button.resize.size" + size);
		}
        else {
			sizeMsg = Component.translatable("handbook.gui.lexicon.button.resize.verybig.message");

			for (int i = 0; i < size - lastSize; i++) {
				sizeMsg = Component.translatable("handbook.gui.lexicon.button.resize.verybig.container", sizeMsg);
			}
		}
