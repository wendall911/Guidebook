...
import guidebook.mixin.client.AccessorMultiBufferSource;
...
public class MultiblockVisualizationHandler {
	...
	public static void onRenderHUD(GuiGraphics graphics, DeltaTracker deltaTracker) {
		if (hasMultiblock) {
			...
			if (timeComplete > 0) {
				...
				graphics.drawCenteredString(mc.font, Component.translatable("guidebook.gui.lexicon.structure_complete"), x, top + height - 10, 0x00FF00);
			...
			if (!isAnchored) {
				graphics.drawCenteredString(mc.font, Component.translatable("guidebook.gui.lexicon.not_anchored"), x, top + height + 8, 0xFFFFFF);
			} else {
				...
				if (timeComplete == 0) {
					...
					if (blocksDone == blocks && airFilled > 0) {
						progress = I18n.get("guidebook.gui.lexicon.needs_air");
						color = 0xDA4E3F;
						mult *= 2;
						posx -= width / 2;
						posy += 2;
	...
	private static class GhostRenderLayer extends RenderType {
		...
		private GhostRenderLayer(RenderType original) {
			super(String.format("%s_%s_ghost", original.toString(), GuidebookAPI.MODID), original.format(), original.mode(), original.bufferSize(), original.affectsCrumbling(), true, () -> {
