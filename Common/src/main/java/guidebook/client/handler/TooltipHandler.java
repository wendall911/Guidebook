	...
	public static void onTooltip(GuiGraphics graphics, ItemStack stack, int mouseX, int mouseY) {
		...
		if (mc.player != null && !(mc.screen instanceof GuiBook)) {
			...
			if (lexSlot > -1) {
				...
				if (GuidebookConfig.get().useShiftForQuickLookup() ? Screen.hasShiftDown() : Screen.hasControlDown()) {
					lexiconLookupTime += ClientTicker.delta;

					int cx = x + 8;
					int cy = tooltipY + 8;
					float r = 12;
					float requiredTime = GuidebookConfig.get().quickLookupTime();
				...
				Component key = Component.literal(GuidebookConfig.get().useShiftForQuickLookup() ? "Shift" : mac ? "Cmd" : "Ctrl")
