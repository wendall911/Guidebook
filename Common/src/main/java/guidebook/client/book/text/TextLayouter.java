...
import guidebook.api.GuidebookConfigAccess.TextOverflowMode;
...
public class TextLayouter {
	...
	public void layout(Font font, List<Span> spans) {
		...
		} while (mode == GuidebookConfigAccess.TextOverflowMode.RESIZE && getOverflow() * getScale() > 1 && adjustScale());
		// if TextOverflowMode is TRUNCATE, yeet everything below baseline
		if (mode == GuidebookConfigAccess.TextOverflowMode.TRUNCATE) {
